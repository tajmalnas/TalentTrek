...
const MCQsTestPage = ({ courseData }) => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "hidden" && testStarted) {
        ...
        alert(
          "You are not allowed to change tabs. If tab is changed for more than 5 seconds cheating will be considered"
        );
        setInactivityTimer(
          setTimeout(() => {
            alert(
              "You've been inactive. AI proctoring round is over due to cheating. Your test will be automatically submitted."
            );
            handleSubmit();
          }, 5000)
        );
  ...
  return (
    <div className="transform relative top-0 text-slate-200">
      {!testStarted && (
        <button
          onClick={handleStartTest}
          className="px-4 py-2 bg-blue-500 m-4 text-slate-200 rounded-md hover:bg-blue-600"
        >
          Start Test
        </button>
      )}
      ...
      <button
        ...
        className="px-4 py-2 bg-blue-500 text-slate-200 rounded-md hover:bg-blue-600"
        disabled={submitted}
      >
        Submit
      </button>
