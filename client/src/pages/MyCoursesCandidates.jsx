...
const MyCoursesCandidates = () => {
  ...
  const fetchCourses = async () => {
    try {
      axios
        .post("/course/my-course-candidates", {
          ...
          courses.map((course, index) => {
            return (
              <div
                ...
                className="bg-[#191b2e] p-4 rounded-xl shadow-lg transform hover:scale-105 transition duration-500"
              >
                <div>
                  <img
                    src={course.image}
                ...
                <div>
                  <h1 className="mt-4 text-slate-200 text-2xl tracking-wide font-bold cursor-pointer">
                    {course.name}
                  </h1>
                </div>
                <div className="">
                ...
                <button
                  className="mt-4 text-lg font-medium w-full text-slate-200 bg-indigo-600 py-2 rounded-xl shadow-lg"
                  onClick={() => navigate("/my-courses/" + course._id)}
                >
                  View Course
                </button>
