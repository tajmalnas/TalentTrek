...
const ExploreCourses = () => {
    ...
    const fetchALlCourses = async() => {
        try{
            axios.get('/course/get-all-courses').then((res) => {
    ...
    const enrollInCourse = async(courseId) => {
        try{
            axios.post('/course/enroll-in-course', {
                courseId,
        ...
        <div class="md:grid md:grid-cols-2 lg:grid-cols-3 gap-6 space-y-4 md:space-y-0">
            {
                courses.length > 0 ? courses.map((course, index) => {
                    return(
                        <div key={index} class="bg-[#191b2e] px-5    pt-6 pb-2 rounded-xl shadow-lg transform hover:scale-105 transition duration-500">
                          <h3 class="mb-3 text-xl font-bold text-indigo-600">Beginner Friendly</h3>
                          <div class="relative">
                            <img class="w-full rounded-xl h-40 object-cover" src={course.image} alt="Colors" />
                            <p class="absolute top-0 bg-yellow-500 text-white font-semibold py-2 px-4 rounded-br-lg rounded-tl-lg flex justify-center item-center"><IndianRupee size={18} /><span>{course.price}</span></p>
                          </div>
                          <h1 class="mt-4 text-white text-xl font-bold cursor-pointer">{course.name}</h1>
                          <div class="my-4">
