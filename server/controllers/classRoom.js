...
export const addVideo = async (req, res) => {
  const { classRoomCode, videoTitle, videoUrl, videoDesc } = req.body;
  if (!classRoomCode || !videoTitle || !videoUrl || !videoDesc) {
...
export const enrollCandidate = async (req, res) => {
  ...
  if (classroom.creator == candidateId) {
    return res
      .status(400)
      .json({ msg: "You are the creator of this classroom" });
  }
  if (classroom.students.includes(candidateId)) {
    return res
      .status(400)
      .json({ msg: "You are already enrolled in this classroom" });
  }
...
export const getEnrolledStudents = async (req, res) => {
  const { classRoomCode } = req.body;
  const classroom = await ClassroomModel.findById(classRoomCode).populate(
    "students"
  );
  if (!classroom) {
    return res.status(400).json({ msg: "Classroom not found" });
...
export const addFinalQuestions = async (req, res) => {
    const { classRoomCode, inputQuestion, optionA, optionB, optionC, optionD, correctOption } = req.body;
    if (!classRoomCode || !inputQuestion || !optionA || !optionB || !optionC || !optionD || !correctOption) {
        return res.status(400).json({ msg: 'All fields are required' });
    }
    try{
        const classroom = await ClassroomModel.findById(classRoomCode);
        ...
        await classroom.save();
        return res.json({msg: 'Question added successfully', classroom: classroom});
    }
    catch(err){
        return res.status(500).json({msg: err.message});
    }
...
export const getFinalQuestions = async (req, res) => {
  const { classRoomCode } = req.body;
  try {
    const classroom = await ClassroomModel.findById(classRoomCode);
    if (!classroom) {
    ...
    return res.json(classroom.test);
  } catch (err) {
    return res.status(500).json({ msg: err.message });
  }
};
